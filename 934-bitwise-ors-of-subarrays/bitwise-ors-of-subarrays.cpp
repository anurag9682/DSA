...
public:
    int subarrayBitwiseORs(vector<int>& arr) {
        ...
        // This set stores the distinct ORs of all subarrays ending at the
        // previous position.
        std::unordered_set<int> current_ors;

        for (int x : arr) {
            // `next_ors` will store the ORs of subarrays ending at the current
            // element `x`.
            ...
            // Compute new ORs by extending previous subarrays with the current
            // element x.
            ...
            // For the next iteration, the current results become the previous
            // results.
